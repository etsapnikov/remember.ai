...
class StructureRepairTest {
    ...
    @Test
    fun `имя группы — настоящее слово, а не его обрубок`() {
        // На живом прогоне продукт предложил «выделить раздел „Одн"»: слово
        // «одна» после усечения стало основой «одн», и её показали человеку.
        // Имя обязано быть тем, что прозвучало.
        val notes = (0 until 6).map {
            Note("n$it", "ещё одна мысль про маркдаун в заметках номер $it")
        } + (6 until 9).map { Note("n$it", "про дачу и забор отдельно вариант $it") }

        val group = StructureRepair.cluster(notes)!!
        // Имя обязано быть **целым словом** из записи, а не его началом: первая
        // починка дала «Маркд» вместо «маркдаун» и мою же проверку прошла,
        // потому что «маркд» — подстрока.
        val spoken = notes.flatMap { Regex("[а-яё]+").findAll(it.text).map { m -> m.value } }
        assertTrue("имя не звучало целиком: ${group.word}", group.word in spoken)
        // И служебное слово темой не становится.
        assertTrue("служебное слово в имени: ${group.word}", group.word !in setOf("одна", "мысль"))
    }
