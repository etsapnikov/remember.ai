...
class DayLineTest {
    ...
    @Test
    fun `падеж не считается выдумкой`() {
        // Р-25.1. Проверка сравнивала точные словоформы: человек говорит
        // «занимались организацией», модель сжимает в «организация обучения»,
        // и вся строка летела в мусор. В базе владельца день так и остался
        // нетронутой расшифровкой — выглядело как «модель не отработала».
        val said = "ну сегодня занимались организацией обучения на продуктовом сайте"
        assertEquals(
            "организация обучения на сайте",
            DayLine.of(said, "организация обучения на сайте"),
        )
    }
