...
class NoteRepositoryTest {
    ...
    fun `впечатление дня правится, а сказанное остаётся`() = runTest {
        ...
        // в раскрытии как было, это память, а не черновик.
        db.days().insert(
            ai.prinim.prinyal.data.DayEntity(
                date = "2026-08-24",
                audioPath = "",
                transcript = "ну самое главное досидели с игорем до сметы",
                line = "досидели с игорем",
                createdAt = 1L,
            )
        )

        repo.editDayLine("2026-08-24", "  досидели с Игорем до сметы  ")

        val day = db.days().byDate("2026-08-24")!!
        assertEquals("досидели с Игорем до сметы", day.line)
        assertEquals("ну самое главное досидели с игорем до сметы", day.transcript)

        // Пустая правка не стирает строку: пустое впечатление — не ответ.
        repo.editDayLine("2026-08-24", "   ")
        assertEquals("досидели с Игорем до сметы", db.days().byDate("2026-08-24")!!.line)
    }
